...
executeSel0 <- function(iRun, iRun2, verb, isWrite, fileName) {
    ...
    if (isSoleFile == F) {
        ...
        while (iGen2 <= nGen) {
            if(verb) {
                if(iGen2 != nGen) { cat(iGen, ", ") } else { cat(iGen) }
            }
...
readData <- function(iRun, iGen, iGen2) {
    if (isSoleFile == T) {
        ...
        genby <- nPop * nGen
        if (genby == nr1) {
          gen <- rep(1:nGen, each = nPop)
        } else {
          gen <- rep(1:flonr1, each = nPop)
          nTimes <- nPop * nGen - nr1
          newnPop <- nPop * flonr1
          gen[newnPop : nr1] <- rep(nGen, times = nTimes)
        }
    ...
    if(sngl == T) {
        ...
        if (nrow(archive) > 0) {
            if (nrow(archive) == 1) {
                ...
                indicatorArc[iRun2] <<- dominated_hypervolume((as.matrix(archive[, 1:nObj])), t(as.matrix(refPoint)))
            } else {
